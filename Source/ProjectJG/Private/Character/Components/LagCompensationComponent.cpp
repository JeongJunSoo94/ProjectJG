...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABaseCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	HeadBox->SetCollisionResponseToChannel(ECollisionChannel::ECC_GameTraceChannel2, ECollisionResponse::ECR_Block);
		...
		else // didn't hit head, check the rest of the boxes
		{
			for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
			{
				if (HitBoxPair.Value != nullptr)
				{
					...
					HitBoxPair.Value->SetCollisionResponseToChannel(ECollisionChannel::ECC_GameTraceChannel2, ECollisionResponse::ECR_Block);
				}
			}
			World->LineTraceSingleByChannel(
				ConfirmHitResult,
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	for (auto& HitLocation : HitLocations)
	{
		...
		if (World)
		{
			...
			ABaseCharacter* BlasterCharacter = Cast<ABaseCharacter>(ConfirmHitResult.GetActor());
			if (BlasterCharacter)
			{
...
void ULagCompensationComponent::ServerScoreRequest_Implementation(ABaseCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime)
{
	...
	if (Character && HitCharacter && Character->GetCombatComp()->GetEquippedWeapon() && Confirm.bHitConfirmed)
	{
		const float Damage = Confirm.bHeadShot ? Character->GetCombatComp()->GetEquippedWeapon()->GetHeadShotDamage() : Character->GetCombatComp()->GetEquippedWeapon()->GetDamage();

		UGameplayStatics::ApplyDamage(
			HitCharacter,
			Damage,
			Character->Controller,
			Character->GetCombatComp()->GetEquippedWeapon(),
...
void ULagCompensationComponent::ProjectileServerScoreRequest_Implementation(ABaseCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	if (Character && HitCharacter && Confirm.bHitConfirmed && Character->GetCombatComp()->GetEquippedWeapon())
	{
		const float Damage = Confirm.bHeadShot ? Character->GetCombatComp()->GetEquippedWeapon()->GetHeadShotDamage() : Character->GetCombatComp()->GetEquippedWeapon()->GetDamage();
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation(const TArray<ABaseCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	{
		if (HitCharacter == nullptr || HitCharacter->GetCombatComp()->GetEquippedWeapon() == nullptr || Character == nullptr) continue;
		float TotalDamage = 0.f;
		if (Confirm.HeadShots.Contains(HitCharacter))
		{
			float HeadShotDamage = Confirm.HeadShots[HitCharacter] * HitCharacter->GetCombatComp()->GetEquippedWeapon()->GetHeadShotDamage();
			TotalDamage += HeadShotDamage;
		}
		if (Confirm.BodyShots.Contains(HitCharacter))
		{
			float BodyShotDamage = Confirm.BodyShots[HitCharacter] * HitCharacter->GetCombatComp()->GetEquippedWeapon()->GetDamage();
		...
		UGameplayStatics::ApplyDamage(
			...
			TotalDamage,
			Character->Controller,
			HitCharacter->GetCombatComp()->GetEquippedWeapon(),
			UDamageType::StaticClass()
		);
	}
}
...
void ULagCompensationComponent::SaveFramePackage(FFramePackage& Package)
{
	...
	if (Character)
	{
		...
		{
			Character->InitializeHitBoxsMap();
		}
